...
def main():
    ...
    while True:
        with sr.Microphone() as source:
            ...
            while True:
                playsound.playsound("C:/Users/THIRAVIAM/Downloads/AI-Powered-Image-Information-Extraction-with-GeminiAI-main/greeting_voice.mp3")
                audio=recognizer.listen(source)
                try:
                    user_input=recognizer.recognize_google(audio, language='en')
                    logging.info(f"user input: {user_input}")
                    response = agent.run(f"{user_input}. "
                                         f"C:/Users/THIRAVIAM/Pictures/c7.jpg")
